...
if ( ! function_exists( 'hendon_core_add_icon_list_item_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function hendon_core_add_icon_list_item_shortcode( $shortcodes ) {
		$shortcodes[] = 'HendonCoreIconListItemShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'hendon_core_filter_register_shortcodes', 'hendon_core_add_icon_list_item_shortcode' );
}

if ( class_exists( 'HendonCoreShortcode' ) ) {
	class HendonCoreIconListItemShortcode extends HendonCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( HENDON_CORE_SHORTCODES_URL_PATH . '/icon-list-item' );
			$this->set_base( 'hendon_core_icon_list_item' );
			$this->set_name( esc_html__( 'Icon List Item', 'hendon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon list item element', 'hendon-core' ) );
			$this->set_category( esc_html__( 'Hendon Core', 'hendon-core' ) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'item_margin_bottom',
				'title'      => esc_html__( 'Item Margin Bottom', 'hendon-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
			...
			$this->set_option( array(
				...
				'group'      => esc_html__( 'Title', 'hendon-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'title_tag',
		...
		public function render( $options, $content = null ) {
			...
			return hendon_core_get_template_part( 'shortcodes/icon-list-item', 'templates/icon-list-item', '', $atts );
		}

...
if ( ! function_exists( 'hendon_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function hendon_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'HendonCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'hendon_core_filter_register_shortcodes', 'hendon_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'HendonCoreListShortcode' ) ) {
	class HendonCoreImageGalleryShortcode extends HendonCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( HENDON_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'hendon_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'hendon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'hendon-core' ) );
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'hendon-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'hendon_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return hendon_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}

...
if ( ! function_exists( 'hendon_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function hendon_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'HendonCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'hendon_core_filter_register_title_layouts', 'hendon_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'hendon_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function hendon_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'hendon-core' );
		
		return $layouts;
	}
	
	add_filter( 'hendon_core_filter_title_layout_options', 'hendon_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'hendon_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function hendon_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'hendon_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'hendon-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'hendon-core' ),
			'author'      => esc_html__( 'Posted by %s', 'hendon-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'hendon-core' ),
			'404'         => esc_html__( '404 - Page not found', 'hendon-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'hendon-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'hendon_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'hendon_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'hendon_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
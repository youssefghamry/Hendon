...
class HendonCoreHeaders {
	...
	public function __construct() {
		...
		add_action( 'hendon_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	function include_elements() {
		...
		foreach ( glob( HENDON_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( HENDON_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
		
		foreach ( glob( HENDON_CORE_INC_PATH . '/header/*/include.php' ) as $header_part ) {
			include_once $header_part;
		}
	}

	function set_variables() {
		$layout_meta = hendon_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'hendon_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_header ? 'hendon_filter_header_template' : 'hendon_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout            = hendon_core_get_post_value_through_levels( 'qodef_header_layout' );
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = hendon_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( HENDON_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->slug . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->slug;
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			hendon_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'hendon-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'hendon-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'hendon-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'hendon-core' ),
		...
		do_action( 'hendon_core_action_additional_header_widgets_area' );
	}
}

HendonCoreHeaders::get_instance();
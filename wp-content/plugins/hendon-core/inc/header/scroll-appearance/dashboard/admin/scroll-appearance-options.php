...
if ( ! function_exists( 'hendon_core_add_scroll_appearance_header_options' ) ) {
	/**
	 * Function that add general options for this module
	 *
	 * @param object $page
	 */
	function hendon_core_add_scroll_appearance_header_options( $page ) {
		...
		if ( $page ) {
			$scroll_appearance_tab = $page->add_tab_element(
				array(
					...
					'icon'  => 'fa fa-cog',
					'title' => esc_html__( 'Scroll Appearance Settings', 'hendon-core' )
				)
			);
			...
			$section->add_field_element(
				array(
					...
					'options'       => apply_filters( 'hendon_core_filter_header_scroll_appearance_option', array( 'none' => esc_html__( 'None', 'hendon-core' ) ) ),
					'default_value' => 'none'
				)
			);
			
			// Hook to include additional options after module options
			do_action( 'hendon_core_action_after_header_scroll_appearance_options_map', $page, $section );
		}
	}

	add_action( 'hendon_core_action_after_header_options_map', 'hendon_core_add_scroll_appearance_header_options', 15 );
}
...
abstract class HendonCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
    ...
    public function register_list_scripts() {
        ...
        $list_scriptes = isset($scripts) ? $scripts : array();
        $list_scriptes = apply_filters( 'hendon_core_filter_registered_list_scripts', $list_scriptes );
        $this->set_scripts($list_scriptes);
    }
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behaviour', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'options'       => hendon_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'options'       => array(
					''      => esc_html__( 'Original', 'hendon-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'hendon-core' ),
				'options'       => hendon_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'hendon-core' ),
				'description' => esc_html__( 'Enter image width in px', 'hendon-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'hendon-core' ),
				'description' => esc_html__( 'Enter image height in px', 'hendon-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'options'       => hendon_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
				'default_value' => '3',
				'group'         => $group,
				'dependency'    => array(
					'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'hendon-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'hendon-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'hendon-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'hendon-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'hendon-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'hendon-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'hendon-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Choose Slide Animation', 'hendon-core' ),
			'options'    => array(
				''     => esc_html__( 'Default (Slide)', 'hendon-core' ),
				'fade' => esc_html__( 'Fade', 'hendon-core' ),
			),
			'dependency' => $dependency,
			'group'      => $group,
			)
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = hendon_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
		
		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ) ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => hendon_core_get_custom_image_size_class_name( $atts['images_proportion'] )
			);
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'hendon-core' );
		...
		$this->set_option( array(
			'field_type'    => 'text',
			'name'          => 'posts_per_page',
			'title'         => esc_html__( 'Posts per Page', 'hendon-core' ),
			'default_value' => '-1',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'hendon-core' ),
			'options'       => hendon_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'hendon-core' ),
			'options'       => hendon_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option ) ) ) {
			...
			do_action( 'hendon_core_action_map_query_options_before_additional', $group );
			
			$additional_params = apply_filters( 'hendon_core_filter_map_additional_query_params', array(
				''       => esc_html__( 'No', 'hendon-core' ),
				'id'     => esc_html__( 'Post IDs', 'hendon-core' ),
				'tax'    => esc_html__( 'Taxonomy Slug', 'hendon-core' ),
				'author' => esc_html__( 'Author Name', 'hendon-core' ),
			) );
			
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'additional_params',
				'title'      => esc_html__( 'Additional Params', 'hendon-core' ),
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'post_ids',
				'title'       => esc_html__( 'Posts IDs', 'hendon-core' ),
				'description' => esc_html__( 'Separate post IDs with commas', 'hendon-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option( array(
					'field_type' => 'select',
					'name'       => 'tax',
					'title'      => esc_html__( 'Taxonomy', 'hendon-core' ),
				...
				$this->set_option( array(
					'field_type' => 'text',
					'name'       => 'tax_slug',
					'title'      => esc_html__( 'Taxonomy Slug', 'hendon-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
				...
				$this->set_option( array(
					'field_type'  => 'text',
					'name'        => 'tax__in',
					'title'       => esc_html__( 'Taxonomy IDs', 'hendon-core' ),
					'description' => esc_html__( 'Separate taxonomy IDs with commas', 'hendon-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
							'additional_params' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'author_slug',
				'title'      => esc_html__( 'Author Slug', 'hendon-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			do_action( 'hendon_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'hendon_core_filter_additional_query_args', $args, $atts );
	...
	public function map_layout_options( $params = array() ) {
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'hendon-core' ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'hendon-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'hendon-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'hendon-core' ),
			...
			do_action( 'hendon_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'hendon-core' ),
				'default_value' => '',
				'group'         => $group_name,
				'dependency'    => array(
					'show' => array(
			...
			do_action( 'hendon_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function load_assets() {
		do_action( 'hendon_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}

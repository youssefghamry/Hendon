...
class HendonCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'hendon_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
		
		// Add title area inline styles
		add_filter( 'hendon_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( HENDON_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( HENDON_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = hendon_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'hendon_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'hendon_filter_title_template' : 'hendon_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = hendon_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = hendon_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = hendon_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = hendon_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile     	  	  = hendon_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding    	  	  = apply_filters( 'hendon_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'hendon_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = hendon_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = hendon_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = hendon_core_get_page_title_image_params();
		...
		$title_color = hendon_core_get_post_value_through_levels( 'qodef_page_title_color' );
